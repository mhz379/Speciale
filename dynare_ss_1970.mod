...
alpha, delta, gamma, beta, sigma, mu, d_bar, g, taup,
...
%%Parameters that change with SS
    ...
    AL=AL_1970;                             
    AK=AK_1970;
    %g = g_1970; hvis den skal være tidsvarierende
...
@#for j in 1:J-51
    ...
    a@{j+1} = (1+r)/(sv@{j})*a@{j} + ((1-taup)*pi@{j}+(1-tau)*w*hc@{j})*(1+AL_growth)^@{j} - c@{j}; %har skrevet J-44 i stedet for J-26
@#endfor
...
q25 = ((x74*fert_25^2*n74)/(n25)*(1/(1+AL_growth)^75))*1/(1+n); %fertilitet opløftet i anden pga. børnebørn.
...
@#for j in 1:J-32
    ...
    pi@{j}=hc@{j}*PI/L; %har ændret J-16 til J-32.
@#endfor
...
disp([num2str(0) '     ' num2str(oo_.steady_state(352,1)) '       ' num2str(oo_.steady_state(350,1)) '     ' num2str(oo_.steady_state(349,1))]) %ændret

disp(['   '])
disp (['Income (Y)  ' 'Consumption (C)  ' 'Agg. Profit (PI)'])
disp([num2str(oo_.steady_state(348,1)) '     ' num2str(oo_.steady_state(351,1)) '          ' num2str(oo_.steady_state(347,1)) '    ']) %ændret
...
cons=oo_.steady_state(75:148,1); %individuelt forbrug.
capi=oo_.steady_state(149:223,1); %individuelle aktiver
for j = 1:J
...
J=75; %ændret fra 57. Skal muligvis være 75.
for j = 1:J
...
x74_scaled_1970=oo_.steady_state(340,1)/(1+AL_growth)^74; %ændret
...
popgen=oo_.steady_state(1:74,1); %ændret
output=oo_.steady_state(348,1); %ændret fra 274 (Y)
personal_debt=0;
J=74; %ændret
for j = 1:J
...
for j = 1:J
	personal_debt=personal_debt+(a_scaled2_1970(j,1)<0)*a_scaled2_1970(j,1)*popgen(j,1)/datasurvival2_1970(1,j); %nettogæld (men er muligvis lig bruttogæld)
end
